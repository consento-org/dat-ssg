...
class Site {
  ...
  async _download ({ signal }) {
    ...
    await intemp.download(this.paths, { opts: ['--recursive'] })
    await intemp.write(['.gitignore'], [
      '.pidfile',
      '.dat'
    ].join('\n'))
    logger.info('Domain: %s', `http${this.config.https ? 's' : ''}://${this.config.domain}`)
    const { images } = await replaceLinks({ dir: intemp.dirname, domain: this.config.domain, newDomain: this.config.newDomain, signal })
    if (images.length > 0) {
      logger.info('Additionally downloading %o', images)
      await intemp.download(images)
    }
    ...
    if (this.config.netlify) {
      ...
      inTmpDomain.writeIfGiven(['netlify.toml'], this.config.netlify.config)
    }
    if (this.config.dat) {
