...
const { open, unlink, rename, mkdtemp: _mkdtemp, lstat, readdir, mkdir } = require('fs').promises
...
async function mkdtemp () {
  if (process.env.CI) {
    return mkdir(join(os.tmpdir(), randomBytes(4).toString('hex')), { recursive: true })
  }
  return _mkdtemp(os.tmpdir())
}
