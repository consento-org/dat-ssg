...
const { open, unlink, rename, mkdtemp, lstat, readdir } = require('fs').promises
...
async function replaceInFile (filepath, { patternMap, signal }) {
  const tmpfile = `${await mkdtemp(os.tmpdir())}/_replace_${Date.now().toString(16)}${Math.random().toString(16)}`
