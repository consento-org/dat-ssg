const { mkdir, readFile, writeFile, mkdtemp, rename: _rename, unlink, access } = require('fs').promises
...
class Site {
  ...
  async update ({ signal } = {}) {
    ...
    try {
      ...
      if (!(await this._commit(isFresh, { signal })) && this.count > 1) {
        return
      }
      ...
      if (this.config.dat && this.config.dat.pushTime) {
        logger.info(`dat:${(await inFolder.cd(this.config.domain).datKey()).toString('hex')}`)
  ...
  async _download ({ signal } = {}) {
    const intemp = operate(await mkdtemp(`${os.tmpdir()}/${name}}`), { signal })
